"""
Kafka Consumer for USGS Earthquakes Real-Time Data.

Subscribes to 'earthquakes-live', validates messages, stores latest/recent records
in Redis, routes malformed records to DLQ 'earthquakes-live-dlq', and commits offsets.
"""
...
from kafka.schemas.earthquake import validate_earthquake_message
from kafka.utils.kafka_utils import TOPIC_EARTHQUAKES, TOPIC_EARTHQUAKES_DLQ
from kafka.utils.redis_client import RealTimeStore

logger = logging.getLogger(__name__)

GROUP_ID_EARTHQUAKE = "environmental-earthquake-consumer"


class USGSConsumer:
    """Consumer service processing live earthquake messages."""
    ...
    def __init__(
        ...
        dlq_topic: str = TOPIC_EARTHQUAKES_DLQ,
        group_id: str = GROUP_ID_EARTHQUAKE,
    ):
    ...
    def push_to_dlq(self, payload: Any, error_reason: str):
        ...
        logger.warning("Routing earthquake event to DLQ [%s]: %s", self.dlq_topic, error_reason)
    ...
    def process_message(self, message_val: Any) -> bool:
        """
        Validate, store, and process single earthquake message.
        ...
        is_valid, reason = validate_earthquake_message(message_val)
        if not is_valid:
        ...
        try:
            stored = self.store.save_earthquake(message_val)
            if stored:
                logger.info(
                    "Successfully processed & stored Earthquake event event_id=%s (mag=%.1f, place='%s')",
                    message_val.get("event_id"),
                    message_val.get("magnitude", 0.0),
                    message_val.get("place", ""),
                )
                return True
            else:
                logger.error("Failed to store Earthquake event in real-time store.")
                return False
        except Exception as e:
            logger.error("Error storing Earthquake event in real-time store: %s", e)
            return False
    ...
    def consume_batch(self, max_messages: int = 10) -> int:
        ...
        try:
            ...
            if processed_count > 0:
                ...
                logger.info("Committed offset after processing %d earthquake records.", processed_count)

        except Exception as e:
            logger.error("Error during USGS consumer poll batch: %s", e)

        return processed_count

    def start_consumer_loop(self):
        """Run continuous consumption loop."""
        logger.info("Starting USGS Consumer loop for topic '%s'...", self.topic_name)
        try:
            while True:
                self.consume_batch()
                time.sleep(0.5)
        except KeyboardInterrupt:
            logger.info("USGS Consumer loop stopped by user.")
